...
HandlingConfig.SectionOrder = table.freeze({ "Receiving", "Appliances", "Furniture", "HeavyGoods", "Industrial", "Secure" })

HandlingConfig.SectionProfiles = table.freeze({
	Receiving = table.freeze({
		RecommendedRig = 0,
		Strength = Vector2.new(9, 16),
		CarrySpace = Vector2.new(8, 14),
		Control = Vector2.new(0.90, 1.08),
		WeightReference = 4.5,
		BulkReference = 4.5,
	}),
	Appliances = table.freeze({
		RecommendedRig = 1,
		Strength = Vector2.new(13, 20),
		CarrySpace = Vector2.new(11, 17),
		Control = Vector2.new(0.98, 1.18),
		WeightReference = 7.0,
		BulkReference = 5.5,
	}),
	Furniture = table.freeze({
		RecommendedRig = 2,
		Strength = Vector2.new(14, 25),
		CarrySpace = Vector2.new(12, 18.5),
		Control = Vector2.new(1.02, 1.32),
		WeightReference = 11.5,
		BulkReference = 10.5,
	}),
	HeavyGoods = table.freeze({
		RecommendedRig = 3,
		Strength = Vector2.new(18, 30),
		CarrySpace = Vector2.new(13, 20),
		Control = Vector2.new(1.08, 1.38),
		WeightReference = 17.0,
		BulkReference = 7.0,
	}),
	Industrial = table.freeze({
		RecommendedRig = 4,
		Strength = Vector2.new(24, 35),
		CarrySpace = Vector2.new(17, 25),
		Control = Vector2.new(1.20, 1.58),
		WeightReference = 25.0,
		BulkReference = 9.0,
	}),
	Secure = table.freeze({
		RecommendedRig = 5,
		Strength = Vector2.new(27, 38),
		CarrySpace = Vector2.new(19, 29),
		Control = Vector2.new(1.34, 1.75),
		WeightReference = 28.0,
		BulkReference = 11.0,
	}),
})

-- Rig tier is only a child-readable summary. Actual handling math always uses
-- Strength / Carry Space / Control directly, so specialization still matters.
...
HandlingConfig.Bands = table.freeze({
	...
	RISKY = table.freeze({
		MinimumRatio = 0.82,
		BaseInstabilityBonus = 0.07,
		InstabilityFloor = 0.10,
		SwayMultiplier = 1.15,
		RecoveryMultiplier = 0.88,
		MovementMultiplier = 0.94,
		StrainFloor = 0.14,
	}),
	DANGEROUS = table.freeze({
		MinimumRatio = 0.65,
		BaseInstabilityBonus = 0.20,
		InstabilityFloor = 0.32,
		SwayMultiplier = 1.40,
		RecoveryMultiplier = 0.66,
		MovementMultiplier = 0.80,
		StrainFloor = 0.44,
	}),
	UNMANAGEABLE = table.freeze({
		MinimumRatio = 0,
		BaseInstabilityBonus = 0.44,
		InstabilityFloor = 0.60,
		SwayMultiplier = 1.78,
		RecoveryMultiplier = 0.38,
		MovementMultiplier = 0.60,
		StrainFloor = 0.76,
	}),
})
...
function HandlingConfig.RequirementsFor(base: any, tunedWeight: number, tunedBulk: number)
	...
	if not profile then
		return table.freeze({ Strength = tunedWeight, CarrySpace = tunedBulk, Control = 1.0 })
	end
	...
	local strength = lerp(profile.Strength.X, profile.Strength.Y, weightAlpha)
	local carrySpace = lerp(profile.CarrySpace.X, profile.CarrySpace.Y, bulkAlpha)
	local control = lerp(profile.Control.X, profile.Control.Y, combined) + shapeBonus + heroBonus

	return table.freeze({
		Strength = rounded(strength, 0.5),
		CarrySpace = rounded(carrySpace, 0.5),
		Control = rounded(control, 0.01),
	})
end
...
function HandlingConfig.RigTierFromStats(strength: number, carrySpace: number, control: number): number
	...
	for _, milestone in HandlingConfig.RigMilestones do
		if strength + 1e-6 >= milestone.Strength
			and carrySpace + 1e-6 >= milestone.CarrySpace
			and control + 1e-6 >= milestone.Control
		then
			result = milestone.Tier
		else
			break
		end
	end
...
function HandlingConfig.RigName(tier: number): string
	for _, milestone in HandlingConfig.RigMilestones do
		if milestone.Tier == tier then return milestone.Name end
	end
	return "STARTER"
end
...
function HandlingConfig.NextRigMilestone(tier: number)
	for _, milestone in HandlingConfig.RigMilestones do
		if milestone.Tier == tier + 1 then return milestone end
	end
	return nil
end
...
function HandlingConfig.Evaluate(
	stats: any,
	definition: any,
	totalWeight: number,
	totalBulk: number,
	...
	local shapeControl = math.min(
		pile.MaximumShapeControlBonus,
		wideCount * pile.ControlPerWideItem + tallCount * pile.ControlPerTallItem
	)
	local effectiveControl = math.max(
		requirements.Control,
		pile.BaseControl + baseInstability * pile.ControlPerBaseInstability + shapeControl
	)
	...
	if spaceRatio <= strengthRatio and spaceRatio <= controlRatio then
		weakness = "CARRY SPACE"
	elseif controlRatio <= strengthRatio and controlRatio <= spaceRatio then
		weakness = "CONTROL"
	end
...
function HandlingConfig.GripFailureSeconds(ratio: number): number
	...
	return math.clamp(
		config.MaxSeconds - shortfall * config.SecondsPerRatioBelowDangerous,
		config.MinSeconds,
		config.MaxSeconds
	)
end
