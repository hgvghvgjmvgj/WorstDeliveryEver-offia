...
local function getStats(player: Player)
	...
	if player:GetAttribute("ProgressionReady") == true then
		local strength = player:GetAttribute("CarryStrength")
		local carrySpace = player:GetAttribute("CarrySpace")
		local control = player:GetAttribute("CarryControl")
		if typeof(strength) == "number" and typeof(carrySpace) == "number" and typeof(control) == "number" then
			return {
				Strength = math.max(0.1, strength),
				CarrySpace = math.max(0.1, carrySpace),
				Control = math.max(0.1, control),
			}
		end
	end
...
local function baseWalkSpeedFor(player: Player): number
	if player:GetAttribute("ProgressionReady") == true then
		local value = player:GetAttribute("MobilityWalkSpeed")
		if typeof(value) == "number" then
			return math.max(CarryConfig.BaseWalkSpeed, value)
		end
	end
...
local function movementSpeedFor(player: Player, state: CarryState): number
	...
	local baseSpeed = baseWalkSpeedFor(player)
	local mobilityBonus = math.max(0, baseSpeed - CarryConfig.BaseWalkSpeed)
	local mobility = CarryConfig.Mobility

	local suppression = math.clamp(
		(ratio - mobility.SuppressionStartWeightRatio)
			/ math.max(0.01, mobility.FullSuppressionWeightRatio - mobility.SuppressionStartWeightRatio),
		0,
		1
	)
...
local function updateStrain(player: Player, state: CarryState, dt: number)
	...
	elseif severity >= tuning.MinimumLoadSeverity then
		local rate = tuning.MaxAccumulationPerSecond * (severity ^ tuning.AccumulationExponent)
		state.Strain = math.min(tuning.Max, state.Strain + rate * dt)
	else
		state.Strain = math.max(0, state.Strain - tuning.ComfortDecayPerSecond * dt)
	end

	if not state.TutorialStrainMessageShown and state.Strain >= tuning.TutorialMessageStrain then
	...
	if newStage ~= state.StrainStage then
		...
		feedbackRemote:FireClient(player, "StrainStage", { stage = newStage })
	end
end
...
local function ensureLayerVisual(state: CarryState, layerIndex: number): LayerVisual?
	...
	local record: LayerVisual = { Part = layerRoot, Weld = weld, Sway = Vector2.zero }
...
local function addVisual(state: CarryState, entry: ItemEntry, index: number, pickupCFrame: CFrame?)
	...
	local definition = ItemConfig[entry.ItemId]
	local duration = CarryConfig.Feel.GrabTweenBaseSeconds + definition.Weight * CarryConfig.Feel.GrabTweenPerWeightSeconds
	TweenService:Create(weld, TweenInfo.new(duration, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { C0 = targetLocal }):Play()
	TweenService:Create(part, TweenInfo.new(duration, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Size = visual.Size }):Play()
end
...
local function sendState(player: Player, state: CarryState, force: boolean?)
	...
	carryStateRemote:FireClient(player, {
		...
		walkSpeed = if state.Humanoid then state.Humanoid.WalkSpeed else baseWalkSpeedFor(player),
	})
end
...
local function initializePlayer(player: Player)
	...
	local state: CarryState = {
		...
		BaseInstability = 0, Strain = 0, StrainLoadSeverity = 0, StrainStage = "None", StrainPhase = 0,
		DynamicSway = Vector2.zero, CurrentSway = Vector2.zero, Phase = 0,
		LastVelocity = Vector3.zero, LastDirection = nil, DangerState = "Stable", CollapseStartedAt = nil,
		LastRecoveryFeedbackAt = -math.huge, TutorialWarningsEnabled = true, TutorialStrainMessageShown = false,
		Character = nil, Humanoid = nil, Root = nil, RigFolder = nil, RigRoot = nil, RigWeld = nil,
		LayerVisuals = {}, MaxLayer = 0, LastGrabAt = 0, VisualAccumulator = 0, StateAccumulator = 0,
		WideItemCount = 0,
	}
	states[player] = state
	...
	player:GetAttributeChangedSignal("CarryPreset"):Connect(progressionChanged)
	player:GetAttributeChangedSignal("CarryStrength"):Connect(progressionChanged)
	player:GetAttributeChangedSignal("CarrySpace"):Connect(progressionChanged)
	player:GetAttributeChangedSignal("CarryControl"):Connect(progressionChanged)
...
local function applyGrabRecoil(player: Player, state: CarryState, itemId: string)
	...
	state.DynamicSway = clampVector2(state.DynamicSway, CarryConfig.Movement.MaxDynamicSway)
end

local function removeEntries(player: Player, state: CarryState, startIndex: number, notice: string, lostFromCollapse: boolean)
	...
	for index, dropped in removed do
		if lostFromCollapse then
			itemService.SpawnCollapseLoss(dropped.ItemId, dropped.StartCFrame, index)
		else
...
local function calculateCollapseSeverity(state: CarryState, risk: number, warningDuration: number): number
	local failure = CarryConfig.Failure
	local danger = CarryConfig.Danger
	local riskSeverity = math.clamp((risk - danger.CollapseRisk) / math.max(0.01, failure.RiskOvershootForMaxSeverity), 0, 1)
	local baseSeverity = math.clamp(
		(state.BaseInstability - danger.MinimumBaseForCollapse)
			/ math.max(0.01, failure.BaseInstabilityForMaxSeverity - danger.MinimumBaseForCollapse),
		0,
		1
	)
	local timeSeverity = math.clamp(
		(warningDuration - danger.CollapseWarningSeconds) / math.max(0.01, failure.ExtraWarningSecondsForMaxSeverity),
		0,
		1
	)
	return math.clamp(
...
local function collapseLossCount(itemCount: number, severity: number): number
	...
	if severity >= failure.SevereSeverityMin then
		...
		local alpha = math.clamp(
			(severity - failure.MinorSeverityMax) / math.max(0.01, failure.SevereSeverityMin - failure.MinorSeverityMax),
			0,
			1
		)
		fraction = failure.NormalLossFraction + (failure.SevereLossFraction - failure.NormalLossFraction) * alpha
	end
	return math.clamp(math.max(failure.MinimumDroppedItems, math.ceil(itemCount * fraction)), 1, itemCount)
end
...
local function partialCollapse(player: Player, state: CarryState, risk: number, warningDuration: number)
	...
	removeEntries(player, state, startIndex, ("NOOO - %d items lost!"):format(count), true)
	feedbackRemote:FireClient(player, "Collapse", { droppedCount = count, severity = severity })
end
...
local function updateLayerVisuals(state: CarryState, risk: number, dt: number)
	...
	for layerIndex, layerVisual in state.LayerVisuals do
		if layerVisual.Part.Parent then
			local layerMultiplier = math.min(stack.MaxLayerVisualMultiplier, 1 + (layerIndex - 1) * stack.LayerSwayAmplification)
			local target = state.CurrentSway * layerMultiplier
			local followRate = math.max(stack.MinimumLayerFollowRate, stack.LayerFollowRate - (layerIndex - 1) * stack.LayerLagPerLevel)
			...
			local shake = math.sin(state.Phase * stack.NearCollapseShakeSpeed + layerIndex * 1.73) * stack.NearCollapseShake * dangerAlpha * topFactor
			local strainAlpha = math.clamp(state.Strain / math.max(0.01, CarryConfig.Strain.Max), 0, 1)
			local strainShake = math.sin(state.StrainPhase * 1.55 + layerIndex * 2.11) * CarryConfig.Strain.VisualShakeAtMax * strainAlpha * topFactor
			local visualX = layerVisual.Sway.X + shake + strainShake
			local visualY = layerVisual.Sway.Y + math.cos(state.Phase * 0.81 + layerIndex) * shake * 0.35 + strainShake * 0.30
			local lean = math.clamp(visualX, -1.35, 1.35) * math.rad(stack.MaxVisualLeanDegrees)
			local pitch = math.clamp(visualY, -1.20, 1.20) * math.rad(stack.MaxVisualPitchDegrees)
			local slipDirection = if visualX >= 0 then 1 else -1
			local slip = slipDirection * stack.NearCollapseSlipStuds * dangerAlpha * topFactor
			local xOffset = math.clamp(visualX, -1, 1) * stack.MaxVisualOffsetStuds + slip
			layerVisual.Weld.C0 = CFrame.new(xOffset, 0, 0) * CFrame.Angles(pitch, 0, -lean)
		end
	end
end
...
local function updateMovement(player: Player, state: CarryState, dt: number)
	...
	if acceleration.Magnitude > movement.AccelerationDeadzone then
		...
		state.DynamicSway += filtered * movement.AccelerationGain * dt * excessScale * swayGenerationMultiplier
	end
	...
	if speed >= movement.TurnMinimumSpeed then
		...
		if previous then
			...
			if math.deg(angle) >= movement.TurnAngleDeadzoneDegrees then
				...
				state.DynamicSway += Vector2.new(sign * angle * movement.TurnGain * wideMultiplier * swayGenerationMultiplier, 0)
			end
		end
	...
	state.DynamicSway = clampVector2(state.DynamicSway, movement.MaxDynamicSway)
	state.Phase += speed * dt * 0.55
	state.StrainPhase += dt * strainTuning.TremorFrequency
	local movementRatio = math.clamp(speed / math.max(CarryConfig.BaseWalkSpeed, humanoid.WalkSpeed), 0, 1.5)
	local oscillationAmplitude = state.BaseInstability * movement.MovementOscillationScale * movementRatio
	...
	local tremorAlpha = math.clamp(
		(state.Strain - strainTuning.TremorStart) / math.max(0.01, strainTuning.Max - strainTuning.TremorStart),
		0,
		1
	)
...
local function animateUnloadVisuals(state: CarryState, unloadCFrame: CFrame)
	...
	for index, entry in state.Items do
		...
		if part and part.Parent then
			...
			local target = unloadCFrame * CFrame.new(x, 1 + row * 0.20, 0)
				* CFrame.Angles(math.rad(12 * row), math.rad(index * 29), math.rad((column - 1.5) * 10))
			...
			task.delay(delaySeconds, function()
				...
				local tween = TweenService:Create(
					...
					{ CFrame = target, Size = originalSize * 0.18, Transparency = 0.55 }
				)
				tween:Play()
...
function CarryService.Unload(player: Player, unloadCFrame: CFrame?): number
	...
	animateUnloadVisuals(state, unloadCFrame or CFrame.new(0, 1, 0))
	table.clear(state.Items)
	clearCarryRig(state)
	state.Weight = 0
	state.Bulk = 0
