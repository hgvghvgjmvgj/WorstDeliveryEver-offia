...
local function addStandardFormat(model: Model, definition: any, visual: Model)
	...
	hitbox.Size = Vector3.new(
		math.max(0.5, boundsSize.X),
		math.max(0.5, boundsSize.Y),
		math.max(0.5, boundsSize.Z)
	)
...
function Importer.Import(cargoId: string, overwrite: boolean?): any
	...
	if existing and overwrite ~= true then
		return {
			...
			Error = "Template already exists. Use REBUILD SELECTED to replace it.",
		}
	end
	...
	local scale = normalizeVisual(visual, definition.TargetBounds)

	-- Important: imported Creator Store assets now keep their source visual identity.
	-- No cargo-specific repaint/remodel hook runs here. In particular, ArcadeCabinet
	-- keeps the original approved Freddy/FNAF texture and geometry unchanged apart
	-- from sanitization, scale, pivot, hitbox and ONE TRIP metadata.
